...
// Works on ALL channels - public, private, announcement, thread
// Whitelist users bypass ALL security rules

const { isWhitelisted } = require('../utils/whitelist');

const spamMap = new Map(); // userId -> { count, first }
const SPAM_LIMIT       = 5;
...
module.exports = {
  ...
  async execute(message, client) {
    ...
    // ── COMMAND HANDLER (works in ALL channels) ───────────────────────────
    ...
    // Whitelisted users bypass all security below
    if (whitelisted) return;

    // ── 1. @EVERYONE / @HERE → BAN (ALL channels) ────────────────────────
    ...
    // ── 2. SPAM → 10 MIN TIMEOUT (ALL channels) ──────────────────────────
    const now  = Date.now();
    ...
    // ── 3. ABUSIVE LANGUAGE → 60 MIN TIMEOUT (ALL channels) ─────────────
